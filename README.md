...
|---|---|---|---|
| **2.8 MB**（ZIP；解压后 7.0 MB） | **35 MB** | **0.5%**（界面本身 0.04%，其余是每分钟一次的状态检查） | **1.8 秒** |

界面只管显示，读写微信的活交给包里的命令行引擎，要检查时才启动、做完就退出；没有常驻后台服务，关掉窗口就不再检查。

<sub>实测 v1.0.8 · Mac16,12 / Apple M4 / macOS 27.2 · 微信 build 269602（App Store 版），窗口打开、无操作 · 2026-09-25。内存为 phys_footprint（活动监视器「内存」列同口径）；CPU 为 180 秒空闲窗口的 CPU 时间 ÷ 墙钟，状态检查每次约 1.4 秒、0.25 秒 CPU，按每分钟一次折算；启动为后台打开 3 次取中位数。原始数据见 [perf/lightweight.json](perf/lightweight.json)。</sub>
