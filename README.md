...
| 安装包 | 空闲内存 | 空闲 CPU | 打开到出结果 |
|---|---|---|---|
| **2.4 MB**（ZIP；解压后 4.3 MB） | **30 MB** | **0.12%**（微信没变时不启动检查引擎；每 30 分钟一次完整检查已折算在内） | **2.8 秒** |

界面只管显示，读写微信的活交给包里的命令行引擎，做完就退出。窗口开着时每分钟只比对一次微信安装包的文件指纹，微信没被更新、打补丁或还原就不启动引擎，另每 30 分钟完整检查一次兜底；没有常驻后台服务，关掉窗口就不再检查。

<sub>实测 v1.0.9 · Mac16,12 / Apple M4 / macOS 27.2 · 微信 build 269627（官网版），窗口打开、无操作 · 2026-09-26。内存为 phys_footprint（活动监视器「内存」列同口径），取 3 个窗口中位；CPU 为 180 秒空闲窗口内界面与子进程的 CPU 时间 ÷ 墙钟，取 3 个窗口平均，完整检查每次约 2.8 秒、0.70 秒 CPU，按每 30 分钟一次折算；启动为后台打开 5 次取中位数。测量时整机负载较高（1 分钟负载均值约 27），同条件下并排实测 1.0.8：空闲 CPU 0.59%、打开到出结果 3.5 秒。原始数据见 [perf/lightweight.json](perf/lightweight.json)。</sub>
